...
## Automatic Agent SBOM Scanning

NannyAgent v0.0.14+ supports **automatic SBOM scanning** triggered from the NannyAPI portal or API. When a scan request is received via the real-time SSE connection, the agent will:

1. **Acknowledge** the scan request to update status in NannyAPI
2. **Execute** the appropriate syft command based on scan type (host, container, or image)
3. **Compress** the resulting SBOM using gzip for efficient upload
4. **Upload** the compressed SBOM to NannyAPI for vulnerability analysis
5. **Report** the scan status and vulnerability counts back to the portal

### Scan Types Supported

| Scan Type | Description | Source |
|-----------|-------------|--------|
| `host` | Full host filesystem scan | `dir:/` with exclusions |
| `container` | Running container scan | `podman:<name>` or `docker:<name>` |
| `image` | Container image scan | `<image>:<tag>` |

### Triggering Scans

Scans can be triggered via:

1. **NannyAI Portal** - Click "Scan SBOM" button on the agent details page
2. **API** - Create a record in the `sbom_scans` collection with `status: "pending"`

```bash
# Trigger a host scan via API
curl -X POST "${NANNYAPI_URL}/api/collections/sbom_scans/records" \
...
For automatic SBOM scanning to work:

1. **syft** must be installed (included in `install.sh` v0.0.14+)
2. Agent must be running in daemon mode (`nannyagent --daemon`)
3. Agent must be connected to NannyAPI real-time endpoint
...
| `max_vulnerabilities_per_scan` | Maximum vulnerabilities stored per scan | `10000` | number |
